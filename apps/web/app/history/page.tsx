...
export default function HistoryPage() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      orders.forEach((order) => {
        ...
        groups[dateStr].push({
          ...order,
          dailyNo: groups[dateStr].length + 1,
        });
      });
  ...
  const handleReprintReceipt = (order: Order & { dailyNo: number }) => {
    const orderAny = order as any;
    const details = orderAny.details || orderAny.items || [];
    ...
    employeeName:
      orderAny.user?.name || orderAny.cashierName || "Kasir Bertugas",
    posName: "POS 1",
    customerName: order.customerName || "",
    queueNumber: order.queueNumber || `#${order.dailyNo}`,
    paidAt:
      orderAny.paidAt ||
      orderAny.updatedAt ||
      orderAny.orderedAt ||
    ...
    paidAmount: orderAny.paidAmount || orderAny.totalPrice || orderAny.total || 0,
    changeAmount: orderAny.changeAmount || 0,
    total: orderAny.totalPrice || orderAny.total || 0,
    items: details.map((detail: any) => {
      const qty = detail.qty || detail.quantity || 1;
      const price = detail.price || detail.menu?.price || 0;

      return {
        menuId: detail.menuId || detail.menu?.id || detail.id,
        name: detail.menu?.name || detail.name || "Menu",
        qty,
        price,
        subtotal: detail.subtotal || qty * price,
      };
    }),
  } as ReceiptData;
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full md:w-1/3 lg:w-1/4 max-w-sm border-b md:border-b-0 md:border-r border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-[#1a1a1a] flex flex-col z-0">
          ...
          <div className="flex-1 flex flex-row md:flex-col overflow-x-auto md:overflow-x-hidden md:overflow-y-auto p-2 md:p-3 gap-2 md:space-y-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
            {isLoading ? (
              <p className="text-gray-500 text-sm p-4 whitespace-nowrap">
                Memuat tanggal...
              </p>
            ) : dateList.length === 0 ? (
              <p className="text-gray-500 text-sm p-4 whitespace-nowrap">
                    ...
                    <div>
                      ...
                      <div
                        className={`text-xs font-semibold ${
                          isActive
                            ? "text-yellow-600 dark:text-yellow-500/80"
                            : "text-gray-500"
                        }`}
                      >
                        {groupedOrders[date].length} Transaksi
                      </div>
                    </div>
